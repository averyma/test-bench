...
def plot_soar(log):
    ...
    adv_acc= np.array(log["pgd20_acc"])[:,2]
    
    train_loss= np.array(log["train_loss_ep"])[:,2]
    test_loss = np.array(log["test_loss"])[:,2]
    adv_loss = np.array(log["pgd20_loss"])[:,2]
        
    reg = np.array(log["train_reg_ep"])[:,2]
    ...
    fig.add_subplot(gs[0,0]).plot(plot_list, adv_acc, "C3", label = "pgd20", linewidth=5.0, marker = "")
    ...
    twin = fig.add_subplot(gs[0,1]).twinx()
    twin.plot(plot_list, adv_loss, "C3", label = "pgd20",  linewidth=5.0, marker = "")
    twin.tick_params(axis='y', labelcolor="C3", labelsize=20)
    
    fig.add_subplot(gs[0,2]).plot(plot_list, reg, "C1", linewidth=5.0, marker = "")

    
    fig.add_subplot(gs[0,0]).set_xlabel("epochs", fontsize = 25)
    fig.add_subplot(gs[0,1]).set_xlabel("epochs", fontsize = 25)
    fig.add_subplot(gs[0,2]).set_xlabel("epochs", fontsize = 25)
    fig.add_subplot(gs[0,0]).set_title("Accuracy", fontsize = 25)
    fig.add_subplot(gs[0,1]).set_title("Loss", fontsize = 25)
    fig.add_subplot(gs[0,2]).set_title("Regularizer", fontsize = 25)
    ...
    # fig.add_subplot(gs[0,0]).set_xlim([0,150])
    # fig.add_subplot(gs[0,1]).set_xlim([0,150])
...
def plot_standard_adv(log):
    ...
    fig.add_subplot(gs[0,1]).plot(plot_list, train_loss, "C1", label = "train", linewidth=5.0, marker = "")
    fig.add_subplot(gs[0,1]).plot(plot_list, test_loss, "C2", label = "test", linewidth=5.0, marker = "")
    fig.add_subplot(gs[0,1]).plot(plot_list, adv_loss, "C3", label = "pgd20", linewidth=5.0, marker = "")
